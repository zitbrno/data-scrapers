...
class Hlasovani extends \ZitBrno\Scrapers\Scraper {
	...
	public function getCislo() {
		...
		preg_match('#<p class="header">.*Hlasování č. ([0-9]+)<br>.*</p>#sU', subject)
	}

	public function getHlasy() {
		$src = static::scrape($this->url);

		preg_match_all('#<td width="20%" align="right" nowrap style="font-size:8pt;font-family:Arial, Arial CE, Courier">(?<person>.+)\s+(?<party>.+): </td><td width="10%" align="left" nowrap style="font-size:8pt;font-family:Arial, Arial CE, Courier">(?<vote>.*)</td>#sU', $src, $matches, PREG_SET_ORDER);
